...
export default {
    ...
    mounted() {
		$(this.$el).datepicker({
			...
			beforeShowDay: date => {
				...
				var flags = this.getFlags(period);
	...
	methods: {
		getFlags(period){
			...
			if(!this.flags[period]){
				$.ajax("/api/v1/query_record_monthly",{
					async : false,
					data : {
						id : this.id,
						period : period
					...
					if(data.code != 0 || !data.data.flags){
						return $.Deferred().reject();
					}						
					this.flags[period] = data.data.flags;
				}).fail(() => {
					this.flags[period] = defaultFlags;
				});	
			}
			return this.flags[period];
		}
	}
}
</script>
